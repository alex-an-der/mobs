...
<head>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.getElementById('tableFilter').value = ''; // Clear filter field on page load
            document.getElementById('insertForm').addEventListener('submit', function(event) {
                event.preventDefault();
                insertRecord('<?=$tabelle?>');
            });
        });
    </script>
</head>
...
// bringt dem Anwender wenig. Es muss daher der unbequeme Weg gegangen werden, die FK foreign keys) zu erkennen und
...
function renderTableSelectBox($db) {
    ...
    echo '<p><form action="data.php" method=j"get">';
        ...
        foreach ($row as $key => $value) {
            if (strcasecmp($key, 'id') !== 0) {
                ...
                if(isset($foreignKeys[$key])) { 
                    ...
                    if ($admin) {
                        echo '<select class="form-controlKlicke auf die Spaltenüberschriften, um danach zu sortieren border-0" style="background-color: inherit;" onchange="updateField(\'' . $tabelle . '\', \'' . $row['id'] . '\', \'' . $key . '\', this.value)">';
        ...
        <form id="insertForm" class="form-inline">
            <?php foreach (array_keys($data[0]) as $header): ?>
                <?php if (strcasecmp($header, 'id') !== 0): ?>
                    <?php if (isset($foreignKeys[$header])): ?>
                        <select name="<?=$header?>" class="form-control mb-2 mr-sm-2">
                            <option value="">---</option>
                            <?php foreach ($foreignKeys[$header]['anzeige'] as $fk_value => $fk_display): ?>
                                <option value="<?=htmlspecialchars($fk_value)?>"><?=htmlspecialchars($fk_display)?></option>
                            <?php endforeach; ?>
                        </select>
                    <?php else: ?>
                        <input type="text" name="<?=$header?>" class="form-control mb-2 mr-sm-2" placeholder="<?=$header?>">
                    <?php endif; ?>
                <?php endif; ?>
            <?php endforeach; ?>
            <button type="submit" class="btn btn-primary mb-2">Neuen Datensatz einfügen</button>
        </form>
    </div>
    <?php endif; ?>
